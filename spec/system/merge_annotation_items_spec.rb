...
RSpec.describe("Merge annotation items", :js) do
  ...
  it do # rubocop:disable RSpec/ExampleLength
    ...
    expect(page).to have_css("#merge_annotation_items", text: "Merge")
    merge_link = page.first("#merge_annotation_items")
    expect(merge_link[:disabled]).to eq("true")
    merge_link.hover
    ...
    expect(merge_link[:disabled]).to eq("false")
    merge_link.click
    sleep(0.2)
    ...
    expect(page).to have_css("#flash_messages", text: "Items merged successfully")
    expect(merge_link[:disabled]).to eq("true")
  end
end
