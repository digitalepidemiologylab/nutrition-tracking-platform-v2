# frozen_string_literal: true

Rswag::Ui.configure do |c|
  # List the Swagger endpoints that you want to be documented through the swagger-ui
  # The first parameter is the path (absolute or relative to the UI host) to the corresponding
  # endpoint and the second is a title that will be displayed in the document selector
  # NOTE: If you're using rspec-api to expose Swagger files (under openapi_root) as JSON or YAML endpoints,
  # then the list below should correspond to the relative paths for those endpoints

  c.openapi_endpoint("/api-docs/v2/swagger.json", "API V2 Docs")
  c.openapi_endpoint("/api-docs/collab/v1/swagger.json", "Collab API V1 Docs")

  # Add Basic Auth in case your API is private
  # c.basic_auth_enabled = true
  # c.basic_auth_credentials 'username', 'password'

  # Dont't try to validate `/api-docs/v2/swagger.yaml`
  # See https://stackoverflow.com/questions/32188386/cant-read-from-file-issue-in-swagger-ui#answer-32244495
  c.config_object["validatorUrl"] = "none"
end
